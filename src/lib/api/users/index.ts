...
export const postSignUp = async (body: T.CreateSignUpRequest): Promise<T.CreateSignUpResponse> => {
  const { data } = await instance.post<T.CreateSignUpResponse>('/users', body);
  return data;
};

export const getMyUser = async (): Promise<T.GetMyUserResponse> => {
  const { data } = await instance.get<T.GetMyUserResponse>('/users/me');
  return data;
};

export const patchMyUser = async (body: T.UpdateMyUserRequest): Promise<T.UpdateMyUserResponse> => {
  const { data } = await instance.patch<T.UpdateMyUserResponse>('/users/me', body);
  return data;
};
...
export const postMyUserProfile = async (
  ...
  const { data } = await instance.post<T.CreateUserImageResponse>('/users/me/image', formData);
  return data;
};
